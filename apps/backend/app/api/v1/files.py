...
from app.services.file_service import delete_project_file, list_project_files, save_project_file
...
def delete_project_file_route(
    project_id: UUID,
    file_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    current_user: Annotated[User, Depends(get_current_user)],
) -> dict[str, object]:
    delete_project_file(db, current_user, project_id, file_id)
    return {"success": True, "data": {"message": "Documento removido com sucesso."}}
